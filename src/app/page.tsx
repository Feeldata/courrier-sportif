  ...
  return (
    <div className="page-stack">
      ...
      <section className="stats-grid" aria-label="Catalogue disponible">
        <div className="stat-card"><TrophyIcon /><strong>{data.counts.competitions}</strong><span>Compétition{data.counts.competitions === 1 ? '' : 's'}</span></div>
        <div className="stat-card"><BallIcon /><strong>{data.counts.matches}</strong><span>Match{data.counts.matches === 1 ? '' : 's'}</span></div>
        <div className="stat-card"><ShieldIcon /><strong>{data.counts.clubs}</strong><span>Club{data.counts.clubs === 1 ? '' : 's'}</span></div>
        <div className="stat-card"><UsersIcon /><strong>{data.counts.players}</strong><span>Joueur{data.counts.players === 1 ? '' : 's'}</span></div>
      </section>
      ...
      <section className="page-section">
        ...
        {data.competitions.length > 0 ? (
          <div className="card-list">
            {data.competitions.map((competition) => <CompetitionCard key={competition.id} competition={competition} />)}
          </div>
        ) : (
          <EmptyState title="Aucune compétition" description="Aucune compétition validée n’est encore disponible." />
        )}
      </section>
